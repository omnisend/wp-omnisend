...
final class ClientTest extends TestCase
{
    ...
    public function test_create_contact_retries_without_the_identifier_the_api_rejected(): void
    {
        ...
        $retried = WP_Http_Test_Stub::$requests[1];
        $this->assertEquals('https://api.omnisend.com/api/contacts', $retried['url']);

        $payload = json_decode($retried['args']['body'], true);
        ...
        $tagging = WP_Http_Test_Stub::last_request();
        $this->assertEquals('https://api.omnisend.com/api/contacts/tags', $tagging['url']);
        $this->assertEquals(array('test-tag'), json_decode($tagging['args']['body'], true)['tags']);
    }
    ...
    public function test_save_contact_retries_without_the_identifier_the_api_rejected(): void
    {
        ...
        $payload = json_decode(WP_Http_Test_Stub::$requests[1]['args']['body'], true);
        $this->assertCount(1, $payload['identifiers']);
        $this->assertEquals('email', $payload['identifiers'][0]['type']);
    }
    ...
    public function test_save_contact_adds_tags_in_a_separate_request(): void
    {
        WP_Http_Test_Stub::queue(WP_Http_Test_Stub::response(200, '{"id":"contact-1"}'));
        $this->queue_tagging_accepted();
